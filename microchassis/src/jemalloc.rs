...
pub fn set_prof_active(value: bool) -> Result<(), Error> {
    ...
    #[allow(unsafe_code)]
    unsafe {
        mallctl::write_mib(&*PROF_ACTIVE_MIB, value).map_err(Into::into)
    }
}
...
pub fn get_prof_active() -> Result<bool, Error> {
    ...
    #[allow(unsafe_code)]
    unsafe {
        mallctl::read_mib(&*PROF_ACTIVE_MIB).map_err(Into::into)
    }
}
...
pub fn prof_dump_file(path: Option<&str>) -> Result<(), Error> {
    ...
    #[allow(unsafe_code)]
    unsafe {
        mallctl::write_mib(&*PROF_DUMP_MIB, ptr).map_err(Into::into)
    }
}
